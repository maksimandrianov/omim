...
BuildingPartsCollector::BuildingPartsCollector(
  ...
  : CollectorInterface(filename), m_cache(cache)
{
}
                                            ...
                                            OsmElement const & element)
{
  ...
  if (!parts.empty())
    WritePair(fb.GetMostGenericOsmId(), parts);
}
    ...
    base::GeoObjectId const & id)
{
  ...
  for (auto const & v : relation.ways)
  {
    if (v.second == "part")
...
void BuildingPartsCollector::WritePair(base::GeoObjectId const & id,
...
BuildingToBuildingPartsMap::BuildingToBuildingPartsMap(std::string const & filename)
{
  ...
  while (src.Size() > 0)
  {
    base::GeoObjectId const outlineId(ReadPrimitiveFromSource<uint64_t>(src));
    ...
    std::string const & filename)
{
  ...
  static std::unordered_map<std::string, BuildingToBuildingPartsMap> maps;
  std::lock_guard<std::mutex> lock(mutex);
  auto const it = maps.find(filename);
  if (it != std::cend(maps))
    return it->second;

  auto const eIt = maps.emplace(filename, BuildingToBuildingPartsMap(filename));
