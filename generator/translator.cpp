...
void Translator::Preprocess(OsmElement *) {}

void Translator::Emit(OsmElement * p)
{
  auto & element = *p;
  if (!m_filters.Need(element))
    return;
  ...
  while (m_featureMaker->GetFeature(feature))
  {
    if (!m_filters.Need(feature))
      continue;
