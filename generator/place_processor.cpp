...
// Returns true if left place is worse then right place.
template <typename T>
...
void FeaturePlace::Append(FeatureBuilder const & fb)
{
  ...
  {
    if (IsWorsePlace(m_fbs[m_bestIndex], fb))
                               ...
                               FeaturePlaces::const_iterator best) const
{
  ...
  for (auto itOuter = start; itOuter != end; ++itOuter)
  {
    auto const & fbs = itOuter->GetFbs();
    for (auto it = std::cbegin(fbs); it != std::cend(fbs); ++it)
    {
      if (!it->IsArea() || !ftypes::IsCityTownOrVillage(it->GetTypes()))
        continue;

      auto const id = it->GetLastOsmId();
      m_boundariesTable->Append(id, indexer::CityBoundary(it->GetOuterGeometry()));
...
void PlaceProcessor::Add(FeatureBuilder const & fb)
{
  ...
  auto const & localityChecker = ftypes::IsLocalityChecker::Instance();
  if (type == ftype::GetEmptyValue() || localityChecker.GetType(type) == ftypes::NONE)
    return;
