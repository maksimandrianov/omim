...
EmitterWorld::EmitterWorld(feature::GenerateInfo const & info)
  : m_cityBoundaryProcessor(std::make_shared<CityBoundaryProcessor>(make_shared<generator::OsmIdToBoundariesTable>()))
...
bool EmitterWorld::Finish()
{
  for (auto const & feature : m_cityBoundaryProcessor->GetFeatures())
    m_worldMapper->RemoveInvalidTypesAndMap(const_cast<FeatureBuilder1 &>(feature));
