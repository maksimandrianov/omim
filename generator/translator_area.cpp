...
{
public:
  RelationCollector(CollectorCollection & collectors) : m_collectors(collectors) {}
...
TranslatorArea::TranslatorArea(std::shared_ptr<EmitterInterface> emitter, cache::IntermediateDataReader & holder,
...
void TranslatorArea::Preprocess(OsmElement * p)
{
  // Here we can add new tags to the elements!
  m_tagReplacer(p);
  m_tagAdmixer(p);
  m_osmTagMixer(p);
  CollectFromRelations(p);
}

void TranslatorArea::CollectFromRelations(OsmElement const * p)
{
